...
**双层**：**核心图**（概念 + 事实，有界）/ **事件层**（事件，**事件 → 核心方向**绑入、**核心不反查**、不聚类、时间倒排截断、帧相对——**每 universe 一条独立扁平时间轴**：现实=真实 ISO、作品=作品纪年，universe 间时间不互染、不引入递归节点）。**叙事时间线**为查询期组装的虚拟视图（某 universe 事件层按 timestamp 升序；`narrative_timeline` / agent `timeline` 工具），**不落图**；Phase 1 仅数字年纪年可排（混合纪年 Phase 2 归一化）。**载重命根**：核心任何节点不反查事件（尤其"用户 / 我"——否则最热节点把全部事件漏回核心、撑爆活跃视图）；**核心侧 `get_relations` 过滤事件边**落实此规则（见「边方向」）。**产生方式**：概念 / 事实靠 LLM 语义抽取；source 由**规则**入库、不经 LLM；**事件按 universe 分流**——现实摄入事件（`__reality__`，记录摄入行为）走规则、**不经 LLM**，作品叙事事件（`universe=work`，作品文本里带纪年的叙述发生）走 **LLM 抽取**（`extract_work_events` 产 `WorkEventDraft`，仅 `work_id` 非空时启用）、MUST NOT 由规则产生。
