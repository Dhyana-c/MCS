...
SYSTEM_PROMPT = (
    ...
    "MAY 含固定历史时间（如「1976 年」）作命题属性。\n"
    "- 带时间的发生（如「今天去按摩」）→ 抽成去时间化的事实（如「用户去按摩」），"
    "时间归事件层、MUST NOT 进事实 content。\n"
    "- MUST NOT 抽事件性命题（如「X 的情况」「某次 X 导致 Y」）或偏好/模式（如「喜欢 X」）"
    "作概念/事实——事件性归事件层、偏好靠概念被多事件背书涌现。\n"
)
...
USER_TEMPLATE = (
    ...
    "**content 不放任何时间词**（概念零时间；事实仅允许固定历史时间如 1976，禁相对/单次时间）；"
    "带时间发生抽成去时间化事实（如「今天去按摩」→ 事实「用户去按摩」）。"
