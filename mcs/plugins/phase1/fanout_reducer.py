...
class FanoutReducerPlugin(CompactionPluginInterface):
    ...
    def _select_batch(self, node: Node, neighbors: list[Node]) -> list[Node]:
        ...
        不变量成立时（node + 全部邻居 ≤ T），直接返回全部邻居；
        不变量被违反时（历史遗留超量节点），贪婪选取一批（循环处理直到收敛）。
        至少选取 2 个邻居以保证聚类可行。
        """
        tb = self.token_budget
        if tb is None:
            return neighbors[:min(32, self.max_community_size)]
        # 不变量成立（全部邻居一次装得下）→ 返回全部
        if not self._exceeds_budget(node, neighbors):
            return neighbors
        # 不变量违反：贪婪选取一批（口径与 _exceeds_budget 一致）
        logger.warning(
            "节点 '%s' 邻域超预算（%d 个邻居），贪婪选取一批",
            node.name, len(neighbors),
        )
        batch: list[Node] = []
        for nb in neighbors:
            if self._exceeds_budget(node, [*batch, nb]):
                break
            batch.append(nb)
        # 至少选取 2 个（否则无法聚类）
