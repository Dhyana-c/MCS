...
MCS 是图结构的"基于语义的记忆检索 + 写入"引擎。它不预设上层用法：
...
| [`write-pipeline`](write-pipeline/spec.md) | 写流程 6 段管线（前置 → 关联定位 → 提取 → 判定 → 应用 → 压缩），写复用读的对称性，决策清单与图更新的分离契约 |
| [`plugin-protocol`](plugin-protocol/spec.md) | 5 类插件接口（Entry / Trim / Arbitration / Postprocess / Compaction），插件链的优先级 / 累积 / 短路语义 |
...
OUTPUT: result (默认 List[Node])       OUTPUT: 图状态更新
```
