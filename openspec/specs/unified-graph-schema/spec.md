...
重复的同名 / 同义概念 SHALL 由读写共同触发收敛：创建时对齐、之后被写 / 读触及时（read-repair）、聚类时合并。同名 SHALL 可由字面匹配当场识别，但 MUST NOT 仅凭同名盲并（同名未必同义，需消歧）。事实 MUST NOT 走合并去重（合并会断背书 / 互斥）；同一事实按"同主 · 同宾 · 同说法"对齐去重。完全未被触及 / 聚类的长尾残留 SHALL 由可选的后台维护扫描兜底。
