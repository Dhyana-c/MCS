...
The system SHALL expose the pure data models in `mcs.entities`. `Edge` SHALL 以 `type`（**取代 `kind` + `label`**）承载结构 / 语义类别（当前仅 `关联` / `互斥`），保留 `priority`、`extensions`（`extensions` 与 `Node.extensions` 对称，供 `EdgeExtensionInterface` 插件挂字段）。`Node` SHALL 有 `node_class`（`概念` / `事实` / `事件` / `source`）作为节点分类轴、`universe`（世界归属，默认 `"__reality__"`）作为与 `node_class` 并列的结构行为归属轴（控制合并 / 互斥 / 聚类 / 载重过滤 / 事件层分立边界，对**全部 `node_class`**（含事件）生效）；`hub` 降为标记（存于 `extensions`，不作为 role / 节点类）。

#### Scenario: graph 模块导出图数据类

- **WHEN** 加载 `mcs.entities.graph`
- **THEN** 存在 `Node` dataclass（字段 `id, name, content, node_class, universe, extensions`）、`Edge` dataclass（字段 `source_id, target_id, id, type, priority, extensions`）、`Subgraph` dataclass（字段 `focus_id, nodes, edges`）
- **AND** `Node.universe` 默认 MUST 为 `"__reality__"`（向后兼容，不传即现实世界）
- **AND** `Edge` MUST NOT 有 `kind` / `label` 字段；`type` MUST ∈ 已登记类型（当前 `关联` / `互斥`）
- **AND** `Node` MUST NOT 以 `role` 作分类轴；`hub` MUST 仅为标记

#### Scenario: Node 不传 universe 默认现实世界

- **WHEN** 不传 `universe` 构造 `Node`
- **THEN** `node.universe` MUST 为 `"__reality__"`
- **AND** 各实例默认值 MUST 独立（不共享可变默认）
