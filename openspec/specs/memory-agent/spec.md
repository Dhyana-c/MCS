...
`MemoryAgent` SHALL 通过 LLM tool calling 执行 ReAct 循环：LLM 返回 `tool_calls` 时执行工具并把结果回灌为 `tool` 消息；无 `tool_calls` 时返回最终答复文本；达到 `max_turns` 回退为兜底文本。**开启 `context_budget` 时，每轮组装 MUST 经会话上下文自治机制**（硬闸 / 折叠 / 预算注入——见「agent 会话上下文自治」requirement）；未开启时行为不变。开启预算时 system prompt SHALL 追加动态预算段。
...
- **AND** 开启 `context_budget` 时 MUST 先尝试一次收尾轮（见「agent 会话上下文自治」）
