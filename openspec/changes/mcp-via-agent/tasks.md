...
- [ ] 1.1.3 新增 `from mcs_agent.builder import create_agent` / `from mcs_agent.loop import MemoryAgent` / `from mcs_agent.llms import AGENT_LLM_REGISTRY`（`PROVIDER_TO_MCS_LLM` 若反推未用到则不 import）
...
### 1.4 构造期预热 client（design D7）
- [ ] 1.4.1 `__init__` 成功构 agent 后：`backend = self._agent.llm; if hasattr(backend, "_get_client"): backend._get_client()`（单线程预热，防首次并发 double-init）

### 1.5 `build_fastmcp` 改 `asyncio.to_thread`
...
## 2. Phase 2 — `mcs_agent/builder.py` 兜底修复（design D6，核心 bug）

- [ ] 2.1 `AgentBuilder.build`：步骤 2 `memory = MemoryStore(build_fn)` 后用 `try/except` 包步骤 3/4，失败时 `try: memory.shutdown() except: logger.warning(...)` 再 `raise`（~5 行，零行为变化）
- [ ] 2.2 全量测试确认成功路径不触发、失败路径多了确定性清理
