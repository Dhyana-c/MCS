...
**Reason**: 后端切 `mcs_agent`，`query` 不再走 `mcs.query` + `render_query_result` 渲染，改为委托 `agent.chat` 返回自然语言答复。渲染委托与「Subgraph 渲染 / 字符串透传」scenario 不再适用。

**Migration**: 见 ADDED「query 工具委托 agent 多步探索」。`query` 返回值从结构化节点 / 边渲染文本 → agent 自然语言答复（含 `[id:...]`）；依赖旧格式的客户端需适配（当前无人用 mcs_mcp，无实际影响）。
