...
- 已建 8 份 delta（走 `openspec archive` apply）：`lightweight-query`（收纳存活）/ `plugin-protocol`（删 3 接口 + Preprocess 废弃指向去 QueryPreprocess）/ `mcs-builder`（删 query API + show Reader 段）/ `memory-agent`（删 associate mcs 模式 + render_query_result 复用）/ `result-rendering`（删 render_query_result）/ `phase1-defaults`（SummaryPlugin 保留 + 删 2 默认空 scenario + Idempotency 标注）/ `preprocess-plugin`（REMOVED 查询管线阶段 ① + 废弃指向去 QueryPreprocess）/ `bench-utils`（MODIFIED：.env 加载去 runner 场景 + 功能替代改指 `agent_full_run.py`）。
- **2 份整目录删除**（全退役、零存活 requirement；`openspec archive` 不支持空 spec，故手动删 `openspec/specs/{query-pipeline,query-rerank}/` 整目录、不走 delta apply）：`query-pipeline`（18 条全 REMOVED）、`query-rerank`（3 条全 REMOVED）。`mcs-presets` **无 delta**（SummaryPlugin 是 NodeExtension、保留，shared 清单不变）。
