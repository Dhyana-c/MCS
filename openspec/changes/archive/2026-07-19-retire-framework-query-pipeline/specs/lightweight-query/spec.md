# lightweight-query Spec Delta — retire-framework-query-pipeline

> 本 change 把 `query-pipeline` 退役后**存活的图导航 / 遍历原语**收纳到本 capability。Purpose 从"轻量查询"拓宽为"**QueryEngine 图导航 + 遍历原语**——供写管线 ingest 关联定位（`query_nodes`）与记忆 agent 导航（`locate_seeds`）复用；不再提供读查询编排"。
...
> 以下原语从 `query-pipeline` 迁入（query-pipeline delta 已 REMOVED 原条目）。措辞按"写管线 + agent 共用的图底座"语境调整；scenario 取原 load-bearing 子集，完整 scenario 集 impl 期核定。
