...
### 验证（唯一脆性 = 抽取质量）
- [x] 记忆样本量化"概念 vs 事实"抽取准确率、转述嵌套（读到的 / 听说的 / 书里的）不污染时间轴（评测骨架 bench/extraction_quality.py；标注数据集待补充）
- [x] multihop-rag：谓词落点模型回归基线（评测框架已有；merge 后跑回归测指标）
- [x] 现有 db 迁移：新库重建（评测库）；决策：不写迁移脚本，SCHEMA_VERSION=2 新库自动建

---

## 待定决策（实现前拍板）

- [x] 语义边类型起步集（决策：仅关联+互斥，不引入因果/背书边；登记制已实现）
- [x] 登记制实现形态
- [x] 事件 / source 的 `extensions` 字段约定（timestamp / 帧 / 参与者 / source_type）
- [x] 后台维护扫描的触发与算力预算（MCS.run_maintenance + force 参数）
