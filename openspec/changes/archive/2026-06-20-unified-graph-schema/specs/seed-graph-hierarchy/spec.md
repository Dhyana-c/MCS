# seed-graph-hierarchy（delta）

> 边模型与导航改写到单一模型：边仅 `关联` / `互斥`（无 `kind` / `label` / 事实边）；邻接原语 `get_facts` → `get_relations`；导航为核心 BFS 取关联邻居；hub 由**标记**识别（非 `role`）。"抗退化的递归 bounding"（不分批整窗喂、拒绝万能 hub）不变。
