...
> 本次随模型语义更新若干 requirement 标题（MD→JSONL、mcs_agent→mcs_mem）；改名后的新正文见 MODIFIED 段。

- FROM: `### Requirement: 消息实时追加到当天 MD`
- TO: `### Requirement: 消息实时追加到当天碎片`
- FROM: `### Requirement: 碎片文件可人工编辑且不缓存旧版本`
- TO: `### Requirement: pending 碎片可经 API 编辑、confirming/confirmed 只读`
- FROM: `### Requirement: 经 API 编辑当天碎片`
- TO: `### Requirement: 经 API 编辑单条碎片`
- FROM: `### Requirement: 记录消息 API`
- TO: `### Requirement: 记录消息 API 返回碎片 id`
- FROM: `### Requirement: 读取碎片内容 API`
- TO: `### Requirement: 读取碎片返回带状态列表`
- FROM: `### Requirement: 捕获端点经 mcs_agent app 暴露`
- TO: `### Requirement: 捕获端点经 mcs_mem app 暴露`
...
**Reason**: 碎片从「按天 MD 整文件」改为「逐条结构化对象」后，编辑以 `id` 定位单条、不再有整文件覆盖；mtime 乐观锁防的「载入后被 `/note` 追加致覆盖丢行」场景不再适用。
**Migration**: 编辑改用 `PUT /fragments/{id}`（逐条、仅 pending）；JSONL 追加串行化由 `FragmentStore` 内部 `threading.Lock` 保证。
