...
`_dispatch_merge` 在将 `concept.content` 与目标节点 `content` **语义合并**（见 write-pipeline「merge 时 concept content 语义合并到目标节点」）后，MUST 检查目标节点 `content` 长度是否超过配置阈值（默认 500 字）。超过时 MUST 调用 `gen_summary` purpose 对 content 进行 LLM 压缩重写，防止高频合并概念的 content 无界增长。

#### Scenario: merge 后 content 未超阈值

- **WHEN** `_dispatch_merge` 语义合并 content 后 `len(node.content) <= threshold`
- **THEN** write_pipeline MUST NOT 发起额外的 LLM 压缩调用
- **AND** `node.content` 保持语义合并后的值
...
- **THEN** write_pipeline MUST 记录 warning 日志并保留语义合并后的 content
