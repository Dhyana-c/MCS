...
**选择**：放在厂商适配器的 `_raw_call` 方法内（transport 层）。

**理由**：
- `llm-interaction` spec 明确说"重试、限流、缓存 MAY 添加在厂商插件内，是 transport 层关注点"
- 对上层 `call(purpose, ...)` 透明，不改接口签名
- 不同厂商可能有不同的限流策略，适配器内最灵活
- 替代方案（在 `call` 层加装饰器）：所有厂商共享同一重试策略，不够灵活

**实现细节**：
- `_raw_call` 捕获 `openai.RateLimitError`（429）和 `openai.APIConnectionError`（网络错误）
- 指数退避：`base_delay=1s, max_retries=3, factor=2`
- 非 429/网络错误直接抛出
- 重试次数和延迟可通过构造参数配置
...
**选择**：缓存在 `LexicalScorer` 内部，`dict[node_id, tuple[set[str], set[str]]]`（name_tokens, content_tokens）。
