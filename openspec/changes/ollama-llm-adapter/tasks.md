...
- [x] 1.2 `__init__` 读配置：`base_url`(默认 `http://localhost:11434/v1`)、`model`、`timeout`(默认较长)、`max_tokens`(默认 4096)、`api_key`(dummy 占位)
- [x] 1.3 `initialize`：`attach_renderer`；惰性导入 `openai`；本地无凭证 → 默认构造 client（缺 SDK 时安全置 `client=None`）
- [x] 1.4 `_raw_call`：OpenAI 兼容 `chat.completions.create`（system 非空才加 system 消息、传 `max_tokens`），返回 `choices[0].message.content`；失败抛 `LLMCallError`（含"未运行/模型未 pull"清晰提示）；不重试
- [x] 1.5 源码零 prompt 模板
...
- [x] 3.2 `_raw_call` 映射：system 非空→system 消息；空 system 不传；返回 message content（mock client）
- [x] 3.3 默认值：base_url/timeout/max_tokens 回退；本地无凭证默认构造 client
- [x] 3.4 错误：调用失败/连不上/模型未 pull → `LLMCallError` 且不重试
- [x] 3.5 工厂：`knowledge_graph(llm="ollama")` 用 ollama_llm；默认仍 deepseek
- [x] 3.6 惰性导入：缺 `openai` 时仍能 import 类、读 name/interfaces
