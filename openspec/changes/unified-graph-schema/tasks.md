...
- [ ] 聚类裂变 `decide_hub`：语义归纳、事实只重组不合并、允许重叠、滤幻觉 id
- [ ] 边吸收：新中心 H 生成后，子节点 ⊇ H 成员的 X 改连 H
- [ ] 双层：事件层单向绑入、核心不反查（尤其"用户"）、时间倒排截断、帧相对
- [ ] 上下文预算 `W = S + T + R`（`R=T` 默认）

### 写入（write-pipeline）
- [ ] 规则入库：事件按结构直存、source 按类型切分（不经 LLM）
- [ ] 关联节点提取：复用 read 检索已有节点
- [ ] LLM 抽取 + 对齐：概念 / 事实，合并同义 / 判互斥 / 连背书

### 查询（query-pipeline）
- [ ] 核心 BFS（沿关联）+ 四工作区（积累 / 活跃 / visited / frontier）
- [ ] read-repair：读时合并同名 / 同义（过守门、超 T 或需消歧则挂起）
- [ ] 按需 `事实 → 事件` 定向查；`purpose` 字段可见性

### 质量收敛
- [ ] 去重 / 合并的读写触发路径；后台维护扫描（可选）
