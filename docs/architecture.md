...
- **边扩展**：`EdgeExtensionInterface`（`PluginType.EDGE_EXTENSION`，与节点镜像）→ `Edge.extensions`；经 `edges.extensions_json` 持久化，与节点同构地保真编解码（`serialize`/`deserialize`）、**两端反查**（`get_relations`）返回的同一对象带完整扩展、**重组**（fanout `_migrate_edges`）与**快照 / 回滚**保真复制（独立 dict、不共享引用）。未挂边扩展时行为逐字不变。

**字段级渲染可见性**：每个扩展自行决定在某 `purpose` 下是否渲染——`render(node/edge, purpose)` 返回片段=可见、返回 `None`=隐藏、不进渲染文本（点 / 边共用同一判定规则）。边渲染函数 `render_relation_edge` 带 `purpose` 参以支持按 purpose 切换；`render_facts` 透传自身 `purpose`。该可见性仅约束**渲染侧**；守门（铁律一）只估算节点层级视图、不渲染 / 不估算关系边，**不受影响**。

**边渲染 / 估算的查询侧一致性**：`estimate_relation_edge` 委托 `render_relation_edge` 取文本后估算（**不另开估算公式**），随渲染签名透传 `extensions` / `purpose`，使可见扩展片段在两侧一致。此一致性服务**查询侧** token 计数正确性（`render_facts` 渲染事实给 LLM 的真实窗口；及 Phase 2 对查询视图按 `priority` 截断），与守门铁律一无关。
...
│   ├── store.py                 # StoreInterface（统一存储抽象基类）
│   ├── mcs.py                   # MCS 顶层门面（ingest / query / shutdown）
│   ├── builder.py               # 构建器抽象
...
│   ├── write_preprocess_plugin.py  # WritePreprocessInterface（写入管线阶段①）
│   ├── query_preprocess_plugin.py  # QueryPreprocessInterface（查询管线阶段①）
│   ├── postprocess_plugin.py    # PostprocessInterface
...
│   ├── edge_extension.py        # EdgeExtensionInterface
│   ├── storage_schema_ext.py    # StorageSchemaExtensionInterface
│   ├── priority_scorer.py       # PriorityScorerInterface
│   └── seed_selector_plugin.py  # （已废弃，保留一个版本后移除）
│
├── plugins/                     # 插件实现（按 PluginType 分目录）
│   ├── entry/                   # EntryPlugin（alias_entry / hub_fallback）
│   ├── trim/                    # TrimPlugin（priority_trim / llm_seed_selector）
│   ├── postprocess/             # 写入后处理（summary / rerank / idempotency_check）
│   ├── preprocess/              # 写入预处理（source_tracking / cross_doc_linker）
│   ├── maintenance/             # Compaction + Maintenance（fanout_reducer / summary_regen / graph_summary）
│   ├── index/                   # Index（alias_index）
│   └── llm/                     # LLM 适配（deepseek / claude / ollama）
│
├── stores/                      # StoreInterface 实现
...
```
mcs_mcp/            # MCP server 应用包（顶层；MCS 作为 MCP 工具经 stdio 暴露）
mcs_agent/          # 记忆 agent 应用包（顶层；对话 ReAct loop + FastAPI 前端）
docs/               # 理解性文档（本文件所在目录）
openspec/           # 规范与变更管理
