...
> 以及它与 MCP server 的区别。
>
> 个人记忆应用（碎片 / 整合 / 日记 / 召回 / 管理看板）已拆为独立 repo / PyPI 包 **`mcs-mem`**，
> 单向依赖 `mcs_mem` → `mcs_agent` → `mcs`（详见 https://github.com/Dhyana-c/mcs-mem）。
> 本文只讲 `mcs_agent` 核心（ReAct loop / MemoryStore / 工具 / 构造 + 基础 FastAPI app）。
...
个人记忆应用（碎片 / 整合 / 日记 / 召回 / 管理看板）在独立 repo `mcs-mem`，复用 `register_base_routes`
扩展记忆端点 + `/manage.html` + scheduler、挂同一 app——详见 https://github.com/Dhyana-c/mcs-mem。
基础路由：
...
python -m mcs_agent                        # 基础 agent app（/chat /health /graph/expand），默认 http://127.0.0.1:8000
# 记忆应用（碎片/整合/日记/召回/看板）见独立 repo mcs-mem：pip install mcs-mem && python -m mcs_mem
```
