...
### 四·五、离线 LLM listwise 重排实验（同日追加）

> 不重跑 agent：取 USED 契约臂 200 题既有候选序列（词法 doc_rerank 产物，候选集 reached
> 0.975），每题词法 top-30 标题 + 问题喂 deepseek-chat 一次 listwise 重排（json 输出、
> temperature 0、解析失败回退词法），重算指标。脚本
> `scripts/exp_llm_rerank_offline.py`；200 题 17 秒、0 回退、~600 token/次。

| overall | 词法 | **LLM 重排** | Δ | 公开最强参照* |
|---|---|---|---|---|
| hit@2 | 0.595 | **0.735** | +14.0pt | — |
| hit@4 | 0.675 | **0.825** | +15.0pt | 0.792 |
| hit@10 | 0.855 | **0.885** | +3.0pt | 0.904 |
| recall@10 | 0.529 | **0.585** | +5.6pt | — |
| map@10 | 0.344 | **0.450** | +10.6pt | ~0.48 |
| mrr@10 | 0.577 | **0.701** | +12.4pt | 0.675 |

\* Multi-Meta-RAG（arXiv 2406.13213，voyage-02 + LLM 元数据过滤）——**口径不可直接比**：
其为 chunk 级证据匹配 + 全量 2556 query，本表为文档级 + 200 题子集；其元数据过滤吃了
该 bench 合成 query 字面含来源名的构造红利。仅作量级参照。

- inference 类改善最大：mrr 0.513 → **0.721**（+21pt）、hit@4 0.614 → 0.886——此前三臂最弱链成为最强。
- 结论：**"导航找全（图 + agent）→ 模型排序（listwise）"流水线成立**——排序端 10-12pt 的
  缺口一次廉价调用（+1 次/题）基本抹平；剩余瓶颈移到 hit@10 尾部（候选 top-30 截断外的 gold）
  与 evidence 级口径对标。
- 工程注记：`bench/plugins/doc_rerank.py` 既有 `llm_doc_rerank`（框架查询线旧实验）与本实验
  同思路；本脚本差异为 json_object 输出 + top-30 截断 + 离线复算，后续可并入统一实现。
...
4. **排序瓶颈已定位且验证可破**：导航侧 recall@∞ 0.938，词法 doc_rerank 是天花板；
   离线 LLM listwise 重排（§四·五）以 +1 次调用/题把 mrr@10 抬到 **0.701**、map@10 **0.450**
   ——在文档级口径上越过/贴近公开最强参照。USED 采纳率（12%）仍是把"模型排序"并入
   交付路径（零额外调用版）的杠杆。
